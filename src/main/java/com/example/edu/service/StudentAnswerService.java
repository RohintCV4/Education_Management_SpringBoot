...
//	}
	 ...
	 public Map<String, Object> createStudentAnswer(final StudentAnswer studentAnswer) {
		 ...
		 //System.out.println("000000000000000000000000000");
	       ...
	       // StudentAnswer subject=studentanswerRepository.findBySubjectCourseId(studentAnswer.getSubjectCourse()).orElse(null);
...
/*
	        // Retrieve the subject course information
	        Optional<SubjectCourse> subjectCourse = subjectCourseRepository.findById(studentAnswer.getId());
	        
	        if (subjectCourse.isPresent()) {
	            // Check if the student is enrolled in this subject course
	            List<StudentCourse> studentCourses = studentCourseRepository.findByStudentIdAndCourseId(
	                studentAnswer.getId(), subjectCourse.get()
	            );
	            ...
	            if (!studentCourses.isEmpty()) {
	                ...
	                response.put("Message", "Student ID can't access this question");
	            }
	        } else {
	            response.put("Message", "Subject course ID not found");
	        }
